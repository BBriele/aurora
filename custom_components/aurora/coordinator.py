...
class AuroraCoordinator(DataUpdateCoordinator[AuroraCoordinatorData]):
    ...
    def async_shutdown_timer(self) -> None:
        """Cancel the pending timer and all listeners."""
    ...
    def _handle_fire(self, _now: datetime) -> None:
        """Timer fired → enter RINGING (Phase 1 wires the output adapters)."""
        if self._next is None:
            self._rearm()
            return
        self._active_alarm_id = self._next.alarm_id
        self._state = AuroraState.RINGING
        _LOGGER.info("Aurora alarm '%s' is ringing", self._active_alarm_id)
        # Phase 1: trigger AudioSink / WakeLight / NotifyChannel adapters here.
        self._rearm()
    ...
    # --- State machine actions (used by services; minimal in Phase 0) -------

    @callback
    def async_dismiss(self) -> None:
    ...
    def async_snooze(self) -> None:
        """Snooze the current ring (Phase 1 schedules the re-ring)."""
        if self._state in (AuroraState.RINGING, AuroraState.MISSION):
            self._state = AuroraState.SNOOZED
            self._publish()

    @callback
    def async_trigger_now(self) -> None:
        """Force a ring immediately (test/manual)."""
        self._state = AuroraState.RINGING
        self._publish()
